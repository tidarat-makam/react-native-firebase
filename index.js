...
class Appplication extends Component {
  ...
  initNotification = async () => {
    ...
    firebase.database().ref('Device1/').update({
      ...
      console.log('sent fcmToken successful');
    }).catch((error) => {
                ...
                <View style={{ flex: 1, flexDirection: 'column', alignItems: 'center'}}>
                    <View style={{ flexDirection: 'row', flexGrow: 0.08, }}>
                      <View style={{ width: (Dimensions.get('window').width)/3, marginRight :30, marginBottom: 10 }}>
                          <Button color={this.state.Buzzer === 1 ? "#4ca2d0" : "#9F9F9F"} onPress={() => this.onBuzzer()}  
                          title="Buzzer" 
                          accessibilityLabel="้เปิดเสียงสำโพง"/></View>   
                      <View style={{ width: (Dimensions.get('window').width)/3, }}>
                          <Button color={this.state.power === 1 ? "#4ca2d0" : "#9F9F9F"} onPress={() => this.switchEngine()}  title="Engine" accessibilityLabel="ดับเครื่องยนต์"/></View>
                    </View>
